...
- 评分仍不修改任何固定基础奖励；动态案件只在成功结算后向本周协调组织追加一次评级反馈。完整规则见[玩家假说与误判恢复](M2-Player-Hypotheses-and-Reasoning-Strain)。
...
- 评分不改变固定便士、物品和知识奖励；动态案件 S/A/B/C/D 评级会对本周协调组织追加 `+3/+2/+1/0/-2` 声望。
