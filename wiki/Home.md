...
- 玩家 Capability 升至 schema 23，旧 schema 15–22 存档自动迁移并回填 M1/M2 状态、三职业班次和神秘暴露。
- 世界加载前创建 schema 23 快照；恢复工具支持 dry-run、清单校验、恢复前安全备份、精确替换与路径穿越拒绝。
- Core/Knowledge/Social/Endgame 四区 dirty mask、生命周期摘要同步和 13 项 Forge GameTest 已进入 CI。
