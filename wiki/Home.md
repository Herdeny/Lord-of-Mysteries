...
- 0.9.1 在世界加载前创建 schema 16 快照，并用正式 DataFix、原始 NBT 备份和 `orphaned_entries` 连接旧 M1/M2 数据。
- 0.9.2 完成 Core/Knowledge/Social/Endgame 四区 dirty mask、登录/重生/维度摘要同步和 3 项 Forge GameTest。
- 每次序列晋升记录特性层、纯度、印记、污染和来源哈希。
- 扮演 v2 新增原则理解、角色过度认同和每日 `/pm reflect` 反思。
- 网络协议升至 8，12 个固定消息 ID 分离状态面板与周期核心摘要。

M0 尚未完成：目录仍需扩至 100 节点，并补齐自动重启/降级回滚与真人死亡/跨维度整体验收。
