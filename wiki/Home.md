...
- 玩家 Capability 升至 schema 28，旧 schema 15–27 存档自动迁移并回填 M1/M2 状态、三职业班次、神秘暴露、特性来源 nonce、远门权限/黑名单及秘偶名单/创建冷却/权威收纳。
- 世界加载前创建 schema 28 快照；恢复工具支持 dry-run、清单校验、恢复前安全备份、精确替换与路径穿越拒绝。
- Core/Knowledge/Social/Endgame 四区 dirty mask、生命周期摘要同步和 18 项 Forge GameTest 已进入 CI。
...
自动化基线由仓库构建结果维护，并覆盖内容图、JUnit、Forge GameTest、精确回滚和两次真实专服启动。M2 的四人八小时真人矩阵仍是里程碑关闭门禁；M3 的完整能力组和多人互补也不能由机器计数替代。
...
五条途径的序列 9–7 已可玩。序列 6–5 新增无面人/秘偶大师、催眠师/梦境行者、阴谋家/收割者、盗火人/窃梦家、记录官/旅行家，共 10 瓶魔药、20 项能力和 20 项扮演事件；五种晋升仪式、旅行家 20 秒双向门/三档权限/端点运维、三槽持久秘偶、非凡特性加工、额外负载与活体析出已经 `code_ready`。完整原典系统、专属生物/材料和真人平衡尚未完成，不能标记 M3 完成。
...
- M3 序列 6–5、五种专属晋升仪式、旅行家命名双向门/三档权限/持久黑名单/端点运维/领地兼容基础、三槽持久秘偶/权威收纳/跨维部署、非凡特性加工与额外负载析出已达 `code_ready`；完整原典能力组、形体记录、具体领地 Mod 适配器、组织门权限、专属资源和组队互补矩阵仍在建设，共享梦境归入 M5。
