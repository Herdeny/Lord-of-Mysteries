...
- 玩家 Capability 升至 schema 25，旧 schema 15–24 存档自动迁移并回填 M1/M2 状态、三职业班次、神秘暴露、特性来源 nonce 和默认队伍远门权限。
- 世界加载前创建 schema 25 快照；恢复工具支持 dry-run、清单校验、恢复前安全备份、精确替换与路径穿越拒绝。
- Core/Knowledge/Social/Endgame 四区 dirty mask、生命周期摘要同步和 15 项 Forge GameTest 已进入 CI。
...
- M3 序列 6–5、五种专属晋升仪式、旅行家双向门/三档权限、非凡特性加工与额外负载析出已达 `code_ready`；完整原典能力组、门命名/黑名单/领地兼容、专属资源和组队互补矩阵仍在建设。
