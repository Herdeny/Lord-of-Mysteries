...
- 当前版本：**`0.9.7-1.20.1`**
- 开发阶段：**v0.9 M2 证据关联与案件恢复**（M2）
- 技术基线：Minecraft **1.20.1** · Forge **47.4.20** · Java **17**
- 最后更新：**2026-07-18 12:57:39 UTC+01:00**（`2026-07-18T11:57:39Z`）
<!-- project-status:end -->
...
- 当前内容图包含 114 节点、160 关系，并输出孤儿、剧透、兼容、本地化与资产报告。
- 玩家 Capability 升至 schema 17，旧 schema 15/16 存档自动迁移并回填 M1 两小时状态。
- 世界加载前创建 schema 17 快照；恢复工具支持 dry-run、清单校验、恢复前安全备份、精确替换与路径穿越拒绝。
...
- 网络协议升至 11，14 个固定消息 ID 分离状态面板、周期核心摘要与调查/证据/推理快照。

自动化已经验证 114 节点内容图、精确回滚和两次真实专服启动。真人两小时节奏、死亡/跨维度与多人长时记录继续作为发布前回归证据，而不再阻塞路线进入 M2。
...
- `/pm case analyze` / `archive` / `recover`：分析当前案件、查看结案档案或在调查板附近恢复关键物品。
