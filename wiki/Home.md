...
能力、扮演、生物、确定性新手营地、十二项保底开局补给、动态下一步提示、遗失引导物恢复、野外风险工具、
九章神秘学手账、教程成就、阶段耗时报告、六项目标 + 四项连续性试炼追踪器、`/pm doctor`、
183 项测试、M1/M2 合同、统一资源完整性、持久化队伍恢复与带运行诊断的专用服务器命令—保存—停服冒烟已经完成；真实一小时生存和平衡记录仍待验收。
...
- 首次登录获得神秘学手账与罗盘；`/pm next` 会按营地、序列、消化和关键材料给出当前唯一优先目标。
- 新手营地补给桶确定性提供坩埚、序列 9/8 必需材料、品质辅料与基础防护，序列 7 毒腺仍需狩猎幻形蛇。
- 手稿或罗盘遗失时使用 `/pm recover` 自助补回；背包已满时依次尝试末影箱和脚下安全掉落。
- 灵体微光、灰烬傀儡、灵视提灯、镇静熏香与可再生材料掉落。
- 二十一段教程/晋升成就、`/pm handbook`、`/pm m1check`、`/pm trial report` 与本地化调查日志共同标记纵切进度。

## 探索、物品与规则

- **新手营地**：世界种子确定，距离出生点约 10–16 区块；罗盘和 `/pm camp` 均可定位，旧世界缺失时可补生成。
- **雾都前哨**：距离出生区约 3–5 区块；`/pm mistcity` 定位，委托板负责接取与结算。
- **调查地点**：`/pm case` 定位废弃教堂和邪教救援营地，旧世界加载对应区块时自动补生成。
- **调查委托**：寻猫引导和“失踪调查小队·第一阶段”可玩，覆盖三名 NPC、记者护送和三波夜袭。
...
- **调查命令**：新增 `mistcity`、`case` 与 `commission list|status|accept|abandon`，并保留全部手账、营地和验收命令。
- **试炼追踪**：自动记录六项核心目标、四项连续性门禁，以及营地 10 分钟、序列 9/8/7 为 25/45/60 分钟的阶段耗时；暂停只累计在线时间。
- **M2 预研**：五途径 9-7、34 个生成资源、3 种委托、3 条任务链、持久队伍账本与离线追赶、神秘学家小屋、真假配方与三解救援。
...
- 中英双语、二十一段教程/晋升成就、十二条随机低语、Capability schema 15、183 项单元/契约测试、统一资源门禁和 Forge 专服 CI 冒烟。
...
- [雾都委托与失踪小队第一阶段](M2-Commissions-and-Mist-City-Outpost)
