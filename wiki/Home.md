...
- 玩家 Capability 升至 schema 27，旧 schema 15–26 存档自动迁移并回填 M1/M2 状态、三职业班次、神秘暴露、特性来源 nonce、远门权限/黑名单及秘偶名单/创建冷却。
- 世界加载前创建 schema 27 快照；恢复工具支持 dry-run、清单校验、恢复前安全备份、精确替换与路径穿越拒绝。
- Core/Knowledge/Social/Endgame 四区 dirty mask、生命周期摘要同步和 17 项 Forge GameTest 已进入 CI。
...
- M3 序列 6–5、五种专属晋升仪式、旅行家命名双向门/三档权限/持久黑名单/端点运维/领地兼容基础、三槽持久秘偶、非凡特性加工与额外负载析出已达 `code_ready`；完整原典能力组、秘偶收纳/跨维控制、具体领地 Mod 适配器、组织门权限、专属资源和组队互补矩阵仍在建设。
