...
- **调查委托**：寻猫引导和“失踪调查小队·序章”可玩，进度、便士余额和冷却跨死亡/重登保存。
...
- **调查命令**：新增 `mistcity` 与 `commission list|status|accept|abandon`，并保留全部手账、营地和验收命令。
- **试炼追踪**：自动记录一小时、营地、晋升、狩猎、两次有效扮演、风险峰值以及死亡/恢复/守护符统计。
- **M2 预研**：五途径 9-7、34 个生成资源、2 种委托、2 条任务链、雾都前哨与调查序章。
...
- 中英双语、十九段教程/晋升成就、十二条随机低语、124 项单元测试和完整 CI 检查。
...
- [雾都前哨、委托与失踪小队序章](M2-Commissions-and-Mist-City-Outpost)
