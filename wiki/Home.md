...
- 玩家 Capability 升至 schema 18，旧 schema 15/16/17 存档自动迁移并回填 M1 状态与空结案归档。
- 世界加载前创建 schema 18 快照；恢复工具支持 dry-run、清单校验、恢复前安全备份、精确替换与路径穿越拒绝。
...
- `/pm case analyze` / `archive` / `debrief` / `recover`：分析当前案件、查看评级与详细复盘，或在调查板附近恢复关键物品。
...
- M2 调查板、三案证据/关联推理、持久化结案评分、每日报纸和前哨三类服务台已完成；正式城市结构、完整经济和三个动态案件仍在建设。
