...
> 当前版本：0.9.11-1.20.1 · v0.9 M2 调查与生活
...
路线会锁定到当前委托并同步给 2–4 名同队玩家；队伍持久账本支持离线追赶，结算或放弃按成员处理。Capability schema 19 保证死亡、重登、
跨维度和服务器保存后仍能恢复。

## 自动验证

- 数据基线：4 个委托、4 条任务链、15 类目标。
- `scripts/check_m2_investigation.py` 保护旧 13 步索引、三解、真假配方和双语键。
- 当前自动化基线：完整 Gradle 测试、v0.9 内容图、统一资源完整性、持久队伍恢复与 Forge 专服运行诊断冒烟。
