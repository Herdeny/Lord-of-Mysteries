...
`pm servercheck`、`list` 和 `save-all flush`；第二次必须保留相同世界种子与 schema 22 marker。Build 工作流会执行同一检查。
...
统一门禁保护 v0.9 设计源、内容图、JSON、双语、模型与注册资源；当前自动化基线为 308 项 JUnit、6 项真实 Forge GameTest，并验证 schema 22 首启迁移快照、Capability Clone、精确特性载荷、两小时状态往返、调查板物理邻近门禁、动态案件确定性/三日轮换/七日组织轮值/六项周指令/八案历史/结案后续/联系人事件/态度/三类组织响应/四段主体日程/关系节点/组织反馈/跨队实例隔离/25 槽现场/配方卷宗绑定/独立记者护送/任务物防复制/队伍扩容续玩/物品恢复/跨日队伍实例对齐、证据关联/玩家假说/误判复议/结案复盘、日报和城市服务契约。
