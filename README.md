...
3. 接取“失踪的调查小队·序章”，调查营地证据、成为任一序列 9 并清除三只神秘威胁。
4. 合成坩埚、灵性草药、占卜水晶与月华水。
5. 按顺序把材料放进坩埚，空手点击开始 1200 tick 炼制。
6. 热源决定温度与魔药品质：完美、完整、瑕疵或污染失败。
7. 服用占卜家魔药，获得 122 灵性上限与三项序列能力。
8. 使用扮演事件消化魔药；重复行为受到新颖度衰减和过度占卜惩罚。
9. 管理污染与失控压力；污染达到 100 会按服务器配置触发失控结局。
10. 消化占卜家魔药后，以深灰灵体之泪、石楠和灵性酒精炼制小丑魔药。
11. 狩猎幻形蛇并晋升魔术师，完成占卜家序列 9-7 纵切。
12. 观众/读心者/心理医生与猎人/挑衅者/纵火家作为 M2 预研路径继续保留。
13. 沿偷盗者或学徒途径晋升至序列 7，验证欺诈/解密或空间/星象团队循环。
14. 用半径 3 八点阵列执行净化封印，验证仪式失败、污染与失控分支。
...
| 染血调查笔记 | 调查营地战利品或任务保底 | 失踪调查小队序章证据，可录入神秘学手账 |
| 烧焦的名单 | 完成失踪调查小队序章 | 链二钥匙线索与调查完成凭证 |
...
- 轻量雾都前哨、委托板、事务所便士账本、两条数据驱动任务链和案件线索物品
- 中英双语、十九段教程/晋升成就、十二条随机低语、知识本地化、基础配方与原版纹理回退
- M1 一小时验收追踪器及委托状态：Capability schema 11 持久化
- v0.8 数据生产线：22 途径目录、首批配方 CSV、34 个自动生成资源和 Gradle 门禁
- M2 数据门禁：2 种委托、2 条任务链、15 类冻结目标类型、跨文件引用与至少双解法校验
- JUnit 5 的 124 项测试覆盖玩家数据、委托 schema、任务推进、货币显示、验收进度、能力、炼药、仪式与引导
...
- 废弃调查员营地与雾都前哨均采用轻量程序生成；完整维多利亚镇区、报社/教堂/事务所 NPC 和结构模板尚未实装。
- 占卜家序列 9–7 代码纵切和验收追踪器已完成，但真实一小时生存/专用服务器平衡验收尚未记录。
- 五途径序列 9-7、两种委托和失踪调查小队序章已完成可玩预研；护送/夜袭后半链、多人一致性矩阵与 schema 冻结仍未实现。
