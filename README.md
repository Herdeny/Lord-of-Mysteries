...
按 v0.8 路线，当前正式里程碑仍是 M1“占卜家序列 9-7 纵切”；现有观众、猎人和
...
- `docs/pathways_master.csv` 维护 22 途径目录，偷盗者和学徒明确标记为 `m2_foundation`，不伪装成可玩内容。
- `docs/recipes_master.csv` 与 `scripts/gen_datapack.py` 生成首批配方和途径目录；Gradle 构建自动校验 26 个生成资源。
...
- M1 一小时验收追踪器：六项目标、三项附加统计、Capability schema 7 持久化
- v0.8 数据生产线：22 途径目录、首批配方 CSV、26 个自动生成资源和 Gradle 门禁
- M2 知识经济入口：四类文书/墨水物品、营地残页和四条调查传闻
- JUnit 5 的 87 项测试覆盖玩家数据、验收进度、手账解锁、能力逻辑、恢复规则、灵性消耗、炼药、仪式、扮演与引导
