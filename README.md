...
“普通人 → 序列 9 观众 → 序列 8 读心者”与
“普通人 → 序列 9 猎人 → 序列 8 挑衅者”三条纵向路径，以及
偷盗者、学徒两条序列 9 可玩预研：
...
- `docs/recipes_master.csv` 与 `scripts/gen_datapack.py` 生成首批配方和途径目录；Gradle 构建自动校验 28 个生成资源。
- 新增空白手稿、神秘墨水、配方残页和雾都晨报；残页在调查营地出现并依次解锁 M2 途径、委托和镇区传闻。
- 观众/猎人 9-8 继续作为 M2 预研；五途径 9-7、雾都、委托和任务链一尚未完成。
...
| `Y` | 妙手空空 / 小空间戏法 | 偷盗者抽样目标掉落；学徒把 6 格内掉落物移动到身边 |
| `U` | 应急脱身 / 稳定灵墨 | 偷盗者向后翻滚；学徒消耗文书材料制作知识副本 |
...
- 坩埚多配方识别：占卜家 9-7、观众/读心者、猎人/挑衅者、偷盗者与学徒九种魔药及交叉污染检测
...
- M1 一小时验收追踪器：六项目标、三项附加统计、Capability schema 8 持久化
- v0.8 数据生产线：22 途径目录、首批配方 CSV、28 个自动生成资源和 Gradle 门禁
- M2 知识经济入口：四类文书/墨水物品、营地残页和四条调查传闻
- JUnit 5 的 95 项测试覆盖玩家数据、验收进度、手账解锁、能力逻辑、恢复规则、灵性消耗、炼药、仪式、扮演与引导

### 当前限制

- 灵视仍使用只对施术者可见的服务端粒子；客户端实体描边渲染尚未完成。
- 占卜家/偷盗者/学徒失控体、幻形蛇、灵体微光与灰烬傀儡已可玩，但暂时复用原版实体模型。
...
- 偷盗者/学徒只有序列 9 可玩预研；序列 8-7、雾都、委托与调查链一仍未实现。
