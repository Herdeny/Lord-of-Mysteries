...
内置追踪器已能持久化记录一小时验收目标，但真实生存与平衡记录尚未通过，因此 M1 仍未关闭。
...
- `/pm trial start` 开始 M1 一小时验收；`status` 查看五项目标，`stop` 暂停并保留记录，`reset` 清空重测。
...
| `/pm trial start` | 重置并开始 M1 一小时验收记录 |
| `/pm trial status` | 查看五项目标、死亡、恢复与守护符统计 |
...
- M1 一小时验收追踪器：五项目标、三项附加统计、Capability schema 6 持久化
- JUnit 5 的 84 项测试覆盖玩家数据、验收进度、能力逻辑、恢复规则、灵性消耗、炼药、仪式、扮演与引导
...
- 占卜家序列 9–7 代码纵切和验收追踪器已完成，但真实一小时生存/专用服务器平衡验收尚未记录。
