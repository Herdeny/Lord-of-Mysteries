...
“普通人 → 序列 9 猎人 → 序列 8 挑衅者 → 序列 7 纵火家”三条纵向路径，以及
偷盗者、学徒两条序列 9 可玩预研：
...
12. 炼制偷盗者或学徒序列 9 魔药，验证行窃/潜影或空间戏法/知识副本循环。
...
- `docs/pathways_master.csv` 维护 22 途径目录，偷盗者和学徒明确标记为 `m2_foundation`。
- `docs/recipes_master.csv` 与 `scripts/gen_datapack.py` 生成首批配方和途径目录；Gradle 构建自动校验 30 个生成资源。
- 新增空白手稿、神秘墨水、配方残页和雾都晨报；残页在调查营地出现并依次解锁 M2 途径、委托和镇区传闻。
- 观众/猎人 9-7 继续作为 M2 预研；五途径 9-7、雾都、委托和任务链一尚未完成。
...
- 偷盗者/学徒序列 9：两种魔药、两种主材料、六项扮演、知识副本、独立失控体与 PvP 防滥用规则
- 坩埚多配方识别：三条 9-7 途径、偷盗者与学徒共十一种魔药及交叉污染检测
...
- 中英双语、十三段教程/晋升成就、十二条随机低语、知识本地化、基础配方与原版纹理回退
- M1 一小时验收追踪器：六项目标、三项附加统计、Capability schema 9 持久化
- v0.8 数据生产线：22 途径目录、首批配方 CSV、30 个自动生成资源和 Gradle 门禁
- M2 知识经济入口：四类文书/墨水物品、营地残页和四条调查传闻
- JUnit 5 的 103 项测试覆盖玩家数据、验收进度、手账解锁、能力逻辑、恢复规则、灵性消耗、炼药、仪式、扮演与引导
...
- 观众/猎人已完成 9-7 可玩预研；偷盗者/学徒仅序列 9，序列 8-7、雾都、委托与调查链一仍未实现。
