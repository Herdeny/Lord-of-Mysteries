...
`project-status.json` 是版本、开发阶段、技术基线和最后更新时间的唯一来源；
`roadmap.json` 是设计版本、当前里程碑、阶段门禁和内容规模目标的唯一来源。
Gradle 构建、README、GitHub Pages 和 GitHub Wiki 必须与两者一致。
...
1. 修改代码、资源、测试和本地化。
2. 数据内容优先修改 CSV 主表或 schema v4 领域 JSON，并运行生成器与内容图。
