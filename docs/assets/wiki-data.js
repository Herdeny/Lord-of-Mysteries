...
LOM.roadmap = [
  ...
  { id: "M1", title: "占卜家序列9可玩 Alpha", state: "active",
    points: ["三能力、占卜可信度与服务端访问控制", "扮演事件、消化公式与反刷取衰减",
      "可交互坩埚、热源温控、四档品质与服药晋升", "污染分级、三种失控模式与失控体原型",
      "永燃火柴盒 + N 键非凡者档案", "待补：仪式状态机、专属失控体、调查营地"] },
...
LOM.entries = [
  ...
  { type: "system", id: "breakdown", name: "失控机制", en: "Loss of Control",
    ...
    tags: ["机制", "失控", "危险", "M1"],
    details: [["触发", "污染达到 100"], ["recoverable", "倒地30s + 生成失控体原型 + 24h精神创伤"], ["config", "recoverable / permanent / death"]],
    long: "失控是踏足非凡的代价。污染达到 100 时，默认 recoverable 模式让玩家倒地 30 秒、生成带专用标记的失控体原型，并施加 24 小时精神创伤（灵性恢复减半）。服务器也可选择永久剥离途径或直接死亡。专属失控体模型、AI 与破碎特性掉落尚待补完。" },
  ...
  { type: "block", id: "lord_of_mysteries:ritual_altar", name: "仪式祭坛", en: "Ritual Altar",
    summary: "仪式结构核心方块。当前已注册、可合成与掉落，状态机交互仍在开发。",
    tags: ["方块", "仪式", "规划中"],
    details: [["硬度", "3.0"], ["爆炸抗性", "6.0"], ["用途", "仪式核心"], ["工具", "需正确工具掉落"]],
    long: "仪式祭坛已作为可合成方块落地，但多方块检测、材料锁定和阶段状态机尚未接线。净化封印仪式目前只有数据 JSON，不能在生存模式执行。" },
  ...
  { type: "artifact", id: "lord_of_mysteries:eternal_matchbox", name: "封印物 · 永燃火柴盒", en: "Eternal Matchbox",
    ...
    details: [["危险等级", "5"], ["主动效果", "放置灵魂火 / 攻击实体"], ["灵体伤害", "对失控体原型造成 12 点魔法伤害"], ["代价", "每次使用 失控压力 +15"], ["耐久", "16"]],
    long: "永燃火柴盒可在合适方块表面点燃灵魂火，也可直接灼烧实体；对带失控体标记的目标造成双倍魔法伤害。每次使用增加 15 失控压力并消耗耐久，5 秒冷却。净化封印仍待仪式状态机落地。" },

  /* 仪式 */
  { type: "ritual", id: "lord_of_mysteries:calm_sealing", name: "净化封印仪式", en: "Calm Sealing Ritual",
    summary: "设计数据已定义的夜间净化仪式；运行时状态机尚未实装。",
    tags: ["仪式", "封印", "数据已定义", "规划中"],
    details: [["环境", "夜晚 + 晴天"], ["材料", "纯净水×3 · 青色兰花×5 · 白蜡烛×8"], ["布局", "祭坛半径3圆阵"], ["失败风险", "低阶灵体 / 污染 / 爆炸"]],
    long: "JSON 已定义环境、材料、阶段和失败结果，用于后续通用 RitualStateMachine。当前版本不会扫描圆阵或消耗材料，资料站将其标记为规划内容，避免与游戏实装状态混淆。" },
