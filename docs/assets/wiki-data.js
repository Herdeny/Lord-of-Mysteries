...
LOM.entries = [
  ...
  { type: "system", id: "lord_of_mysteries:guide_commands", name: "调查指引命令", en: "Investigator Guide Commands",
    summary: "十四组无需管理员权限的自助命令，覆盖阶段、诊断、手账、探索、委托、资料与验收记录。",
    tags: ["系统", "命令", "引导", "M1"],
    details: [["阶段", "/pm guide · /pm m1check"], ["诊断", "/pm doctor"], ["手账", "/pm handbook [1-9]"], ["状态", "/pm status"], ["探索", "/pm camp · /pm mistcity · /pm case"], ["委托", "/pm commission list|status|accept|abandon"], ["资料", "/pm rules · /pm items · /pm bestiary · /pm journal"], ["验收", "/pm trial start|resume|status|verify|stop|reset"]],
    long: "命令由服务端权威执行并发送本地化文本。doctor 会先修复数据再检查定义、协议和世界目标；试炼命令只管理验收记录，不授予途径、序列或物品。" },
  { type: "system", id: "lord_of_mysteries:tutorial_advancements", name: "教程与晋升成就链", en: "Tutorial and Advancement Chain",
    summary: "二十段成就覆盖 M1 主线、四条预研途径晋升、雾都委托与记者救援。",
    tags: ["系统", "成就", "引导", "M1"],
    details: [["起点", "帷幕微动"], ["探索", "跟随指针 / 准备充分的调查员"], ["防护", "写在灰烬中的警告"], ["M1", "占卜家 / 小丑 / 魔术师"], ["M2", "四条预研途径晋升"]],
  ...
  { type: "system", id: "lord_of_mysteries:m1_trial_tracker", name: "M1 一小时试炼追踪器", en: "M1 One-Hour Trial Tracker",
    ...
    details: [["命令", "/pm trial start|resume|status|verify|stop|reset"], ["目标", "一小时 · 营地 · 序列7 · 3次狩猎 · 2次扮演 · 风险峰值25"], ["连续性", "重连 · 重启 · 维度往返 · 死亡恢复"], ["存档", "Capability schema 13"]],
    long: "追踪器只累计在线时长，stop 后 start/resume 会恢复记录。状态在死亡、跨维度、重登与服务器重启后保留；核心 6/6 和连续性 4/4 只提供机器证据，不能替代真实一小时平衡验收。" },
  { type: "system", id: "lord_of_mysteries:server_validation", name: "专用服务器与运行诊断", en: "Dedicated Server Validation",
    summary: "用玩家自检、152 项测试和 CI 专服启动—保存—停服冒烟保护 M1 连续性与 M2 前置基础。",
    tags: ["系统", "专用服务器", "CI", "诊断", "M1"],
    details: [["诊断", "/pm doctor"], ["测试", "152"], ["协议", "6 · 11 个固定 ID"], ["冒烟", "2 委托 · 2 任务链 · Done · 保存停服"]],
    long: "Build 工作流在 clean build 后实际启动 Forge 服务器。脚本确认定义加载和世界就绪，再发送 stop 并要求正常保存。多人矩阵另覆盖同队共享、协调者切换、跨队隔离和人数上限降级。" },
  ...
  { type: "block", id: "lord_of_mysteries:commission_board", name: "雾都委托板", en: "Mist City Commission Board",
    ...
    tags: ["方块", "委托", "雾都", "M2"],
    details: [["交互", "右键"], ["接取顺序", "走失的煤球 → 失踪小队第一阶段"], ["合成", "纸×3 + 云杉木板×4 + 雾都晨报"], ["权限", "普通玩家"]],
  ...
  { type: "system", id: "lord_of_mysteries:commission_runtime", name: "数据驱动委托与任务状态机", en: "Data-Driven Commission Runtime",
    summary: "热重载两种委托和两条任务链，并以十五类冻结目标推进服务器权威步骤。",
    tags: ["系统", "委托", "数据包", "持久化", "M2"],
    details: [["委托", "2"], ["任务链", "2"], ["目标枚举", "15"], ["存档", "Capability schema 13"], ["合作", "同记分板 2–4 人在线共享"]],
    long: "玩家状态保存便士余额、活动委托、任务步骤、记者 UUID、夜袭波次、完成集合和冷却。schema 13 加载时修复非法值与损坏引用；共享任务按最大人数门禁安全降级。" },
  ...
  { type: "system", id: "lord_of_mysteries:missing_squad_prologue", name: "失踪的调查小队·第一阶段", en: "Missing Investigation Squad: Act One",
    ...
    details: [["步骤", "13"], ["地点", "前哨 · 营地 · 教堂 · 邪教营地"], ["NPC", "办事员 · 联络员 · 记者"], ["报酬", "144 便士 · 声望 +10 · 烧焦名单"]],
    long: "原七步序章之后追加六步以保护旧存档进度。2–4 名在线同队玩家可共享目标，奖励仍逐人结算；神秘学家小屋、真假配方与潜入/强攻/占卜三解仍待实现。" },
