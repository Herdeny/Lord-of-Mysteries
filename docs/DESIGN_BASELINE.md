...
- `ContentMetadata` 在途径、15 个现有序列、4 个委托和 4 条任务链中解析并验证 schema v4 元数据。
- `scripts/build_content_graph.py` 建立 116 节点、168 关系的当前实现图，并输出六类机器可读审计报告。
...
- 网络协议为 12；14 个固定消息 ID、295 项 JUnit、6 项 Forge GameTest、精确回滚和双启动专服矩阵组成分层门禁。

## 事实边界

- 当前关系图是 **116 节点 / 164 关系**，M0 的 100 节点退出门槛已经通过。
...
- M0：`done`。schema v4、116 节点内容图、迁移/回滚框架、来源审计与 CI 门禁完成。
- M1：`done`。占卜家 9–7 两小时核心纵切、扮演 v2、特性死亡守恒和基础城市生活闭环完成。
- M2：当前 `active`。调查板、证据推理、玩家假说、可恢复误判代价、三类八槽位动态案件、案件相关人/受影响者、可视物证、四类封存样本、实体三轮取证、七日组织轮值、主体关系/日程和组织声望反馈已完成；继续扩展多角色关系演化、组织反应任务、周级案件池、正式城市与长期经济。
