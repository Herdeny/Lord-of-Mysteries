...
- `scripts/build_content_graph.py` 建立 116 节点、164 关系的当前实现图，并输出六类机器可读审计报告。
...
- 网络协议为 12；14 个固定消息 ID、251 项 JUnit、6 项 Forge GameTest、精确回滚和双启动专服矩阵组成分层门禁。
...
- M2：当前 `active`。调查板、证据推理、玩家假说、可恢复误判代价、三类八槽位动态案件轮换和实体三轮取证已完成；继续生成案件专属主体/受害者、差异化现场陈设，并建设正式城市与长期经济。
