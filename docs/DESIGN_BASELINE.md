...
- 内容定义：schema v4
- SimpleChannel：协议 9（14 个固定消息 ID）
...
- 0.9.2 增加 Core/Knowledge/Social/Endgame 四区 dirty mask、生命周期核心摘要和客户端只读缓存；0.9.3 将 Forge GameTest 扩至 5 项；0.9.4 增加调查板权威快照、邻近门禁并扩至 6 项。
...
- 网络协议升至 9；14 个固定消息 ID、207 项 JUnit、6 项 Forge GameTest、精确回滚和双启动专服矩阵组成分层门禁。
