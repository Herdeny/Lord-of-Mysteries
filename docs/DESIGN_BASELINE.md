...
- `scripts/build_content_graph.py` 建立 116 节点、163 关系的当前实现图，并输出六类机器可读审计报告。
...
- 网络协议为 12；14 个固定消息 ID、248 项 JUnit、6 项 Forge GameTest、精确回滚和双启动专服矩阵组成分层门禁。

## 事实边界

- 当前关系图是 **116 节点 / 163 关系**，M0 的 100 节点退出门槛已经通过。
...
- M2：当前 `active`。调查板、证据推理、玩家假说、可恢复误判代价和三类八槽位动态案件轮换已完成；继续把槽位落为真实 NPC/物证/现场互动，并建设正式城市与长期经济。
