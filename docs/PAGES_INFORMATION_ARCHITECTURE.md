...
- 完整数据为 189 条，其中实际注册内容为 91 物品、7 方块、9 实体；方块物品与方块按 ID 去重后共有 100 条注册记录，另含随代码同步维护的能力、机制、仪式、组织、世界与状态条目。
...
0.9.25 从 `ModEntities` 自动加入 `traveler_door`，图鉴增至 189 条唯一记录；0.9.26 更新旅行家能力、M3 总览、专服验证、schema 和资源条目，展示 20 秒命名双向门、主动穿门同意、三档权限、32 人持久黑名单、出生点保护、三动作领地事件、安全落点、失败退款、反回弹、schema 26、392 项 JUnit、16 项 Forge GameTest 与 9 个实体。具体领地 Mod 适配器和组织门权限仍明确标为后续边界。
