...
1. 当前版本、里程碑与真实门禁状态；
2. 新玩家、M2 玩家、M3 玩家、M4 玩家、服主/贡献者五类入口；
3. 调查—收集—炼制—晋升—扮演—风险六步核心循环；
4. M2 已实现范围、M3 `playable candidate` 边界、M4 24/24件机器完成范围与M5启动边界；
...
- 数据由 `scripts/gen_pages_catalog.py` 从 `ModItems`、`ManagedArtifactKind`、`ModBlocks`、`ModEntities`、12 份组织定义、7 份封印物定义和双语语言资源生成。
- 完整数据为 254 条，其中实际注册内容为 132 物品、7 方块、16 实体；生成数据还包括 12 个组织与 24 件封印物，人工条目只补充能力、机制、仪式、世界与状态说明。
...
- 动态卡片必须同步 447/24 测试、423/1830/788/139 资源、132/7/16 注册表、164/214 内容图和 254 条图鉴；生成数据必须包含 12 个组织和 24 件封印物；
