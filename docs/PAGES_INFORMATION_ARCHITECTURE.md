...
1. 当前版本、里程碑与真实门禁状态；
2. 新玩家、M2 玩家、M3 玩家、M4 玩家、M5 玩家、服主/贡献者六类入口；
3. 调查—收集—炼制—晋升—扮演—风险六步核心循环；
4. M2 已实现范围、M3 `playable candidate` 边界、M4 24/24件机器完成范围与M5首个灵界远征闭环；
5. 五途径定位与一次一途径的序列 9–5 浏览器；
6. 可搜索、可分类、分批渲染的完整图鉴；
7. M0–M5 当前路线与折叠的 M6–M12 远期路线；
...
- 数据由 `scripts/gen_pages_catalog.py` 从 `ModItems`、`ManagedArtifactKind`、`ModBlocks`、`ModEntities`、12份组织、24份封印物、6份灵界天气、12份灵界遭遇和双语语言资源生成。
- 完整数据为311条，其中实际注册内容为156物品、7方块、28实体；生成数据还包括12个组织、24件封印物、6种天气与12类遭遇，人工条目只补充能力、机制、仪式、世界与状态说明。
...
0.9.31 新增独立 M4 玩家入口、组织/封印物进度卡和 M4 机制卡。图鉴生成器修复辅助注册遗漏，并从 12 份组织与 7 份封印物定义生成详情卡；当前为 237 条图鉴、389 JSON、1741 对双语键、713 个静态引用、122 个模型、115 个物品、7 个方块、16 个实体、147 节点/197 关系内容图、439 项 JUnit 和 24 项 Forge GameTest。布局门禁同时验证普通玩家只能看见本人负责/持有的封印物记录，完整位置账本只属于管理员入口。

0.9.32 将图鉴扩展到24份封印物定义与254条唯一记录，并在首页展示M4机器完成和M5启动。当前基线为423 JSON、1830对双语键、788个静态引用、139个模型、132个物品、7个方块、16个实体、164节点/214关系内容图、447项JUnit和24项Forge GameTest。布局合同同时要求周战略、三席实体联络员、六事故档案、24/24状态、M5当前卡片以及普通玩家保管隐私。

0.9.33 将M5首个灵界远征接入首页与图鉴：新增M5玩家入口、独立维度/五投影/六段航路进度说明，从6份天气和12份遭遇定义生成详情卡，并从注册表加入6件远征物品。当前基线为457 JSON、1922对双语键、802个静态引用、145个模型、138个物品、7个方块、16个实体、164节点/214关系内容图、454项JUnit、26项Forge GameTest与284条图鉴。布局合同要求显示多人独立SavedData、安全恢复和共享梦境边界。

0.9.34 将12类遭遇实体化并加入生成蛋，把2–4人明确同意式共享梦境接入首页、图鉴和Wiki；生成器识别`spiritEcology`与`spiritEcologyEgg`辅助注册。当前基线为485 JSON、2011对双语键、815个静态引用、163个模型、156个物品、7个方块、28个实体、164节点/214关系内容图、464项JUnit、28项Forge GameTest与310条图鉴。布局合同要求M5完成态、M6当前卡、双手册入口和同意/恢复边界。

0.9.35 新增M0–M5跨系统独占活动机制卡、`/pm activity`恢复入口与首页恢复卡。当前基线为485 JSON、2031对双语键、819个静态引用、163个模型、156物品、7方块、28实体、164节点/214关系内容图、476项JUnit、29项Forge GameTest与311条图鉴。布局合同额外要求受保护维度、原子拒绝、十二格主持人复检和第29项真实GameTest证据可搜索。
...
- M4 首页必须展示 `/pm m4`、12 个组织、6 类行动、周战略、三席联络员、24/24件封印物、六事故档案、唯一保管/泄漏/回收闭环，以及高精美术/真人长测边界；
- M5 首页必须展示 `/pm spirit`、五类语义投影、六段航路、12类生态遭遇、6种异常天气、安全恢复与共享梦境边界；
- 动态卡片必须同步476/29测试、485/2031/819/163资源、156/7/28注册表、164/214内容图和311条图鉴；生成数据必须包含12个组织、24件封印物、6种天气和12类遭遇；
- 路线首页必须直接显示M0–M5完成态与当前M6，只有M6之后的远期范围进入渐进折叠；
- 恢复入口必须展示`/pm activity`、`shared_dream`/`spirit_world`受保护维度、原子失败不变式和管理员恢复手册；
