...
4. M2 已实现范围、M3 `playable candidate` 边界与 M4 首批 7/24 件封印物边界；
5. 五途径定位与一次一途径的序列 9–5 浏览器；
6. 可搜索、可分类、分批渲染的完整图鉴；
7. M0–M4 当前路线与折叠的 M5–M12 远期路线；
...
- 完整数据为 237 条，其中实际注册内容为 115 物品、7 方块、16 实体；生成数据还包括 12 个组织与 7 件封印物，人工条目只补充能力、机制、仪式、世界与状态说明。
...
- M4 首页必须展示 `/pm m4`、12 个组织、6 类自主行动、7/24 件封印物、唯一保管/泄漏/回收闭环，以及完整 24 件、更多据点/NPC 日程、跨周战略和真人长测边界；
- 动态卡片必须同步 439/24 测试、389/1741/713/122 资源、115/7/16 注册表、147/197 内容图和 237 条图鉴；生成数据必须包含 12 个组织和 7 件封印物；
