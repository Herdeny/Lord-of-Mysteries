...
- 完整数据为 191 条，其中实际注册内容为 92 物品、7 方块、9 实体；方块物品与方块按 ID 去重后共有 101 条注册记录，另含随代码同步维护的能力、机制、仪式、组织、世界与状态条目。
...
0.9.25 从 `ModEntities` 自动加入 `traveler_door`，图鉴增至 189 条唯一记录；0.9.26 更新旅行家能力、M3 总览、专服验证、schema 和资源条目。0.9.27 新增“秘偶大师持久编队”机制条目，图鉴增至 190 条。0.9.28 自动加入“秘偶收纳卷轴”，图鉴增至 191 条，并同步 schema 28、404 项 JUnit、18 项 Forge GameTest、1521 对双语键与 599 个静态引用。形体记录、具体领地 Mod 适配器和组织门权限仍明确标为后续边界，共享梦境归入 M5。

0.9.27 同时修正首页 M3 测试入口、进度卡和路线摘要：三槽持久秘偶与旅行家端点运维列入已实现范围，只有秘偶收纳卷轴/跨维指挥、完整梦境和形体记录继续列为后续。布局门禁会直接拒绝再次把“持久秘偶”误列为未完成。
...
- M3 首页必须展示三槽持久秘偶、`/pm marionette`、旅行家端点运维与 `/pm travel`，同时保留卷轴/跨维指挥等未完成边界；
