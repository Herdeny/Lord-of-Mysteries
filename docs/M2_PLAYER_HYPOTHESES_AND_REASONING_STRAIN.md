...
> 适用版本：`0.9.15-1.20.1` · Minecraft 1.20.1 · Capability schema 20 · 网络协议 12
>
...
- `PlayerMysteryData` schema 19 新增按案件 ID 索引的 `case_hypotheses`；当前 schema 20 继续保留并增加有界动态案件历史。
...
- 282 项 JUnit 覆盖文本消毒、NBT 往返、错误检验、纠正恢复、动态案件伪证复议、差异化样本/并发现场布局/配方卷宗绑定/跨日实例/队伍扩容续玩、周轮值/指令/历史/结案后续/日程/组织反馈、复议冷却、非法载荷、协议快照、结案扣分和 schema 18→19→20 迁移。
- `scripts/check_m2_investigation.py` 固定三种立场、三种状态、160 字上限、3 点负担、4 点压力、10/30 秒冷却、结案扣分、调查板邻近与零奖励修改。
- `scripts/check_resource_integrity.py` 校验 273 个 JSON、1216 对双语键、480 个静态引用、80 个模型和全部注册资源。
