...
> 适用版本：`0.9.17-1.20.1` · Minecraft 1.20.1 · Capability schema 22 · 网络协议 12
>
...
- `PlayerMysteryData` schema 19 新增按案件 ID 索引的 `case_hypotheses`；当前 schema 22 继续保留，并兼容 schema 20 的有界动态案件历史、schema 21 的组织响应状态与新增联系人记忆分支。
...
- 308 项 JUnit 覆盖文本消毒、NBT 往返、错误检验、纠正恢复、动态案件伪证复议、差异化样本/并发现场布局/配方卷宗绑定/跨日实例/队伍扩容续玩、周轮值/指令/历史/结案后续/联系人记忆/三类组织响应/日程/组织反馈、复议冷却、非法载荷、协议快照、结案扣分和 schema 18→19→20→21→22 迁移。
